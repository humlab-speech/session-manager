...
class Session {
    ...
    async runCommand(cmd, env = [], expectJsonResponse = true) {
        ...
        this.app.addLog(
            "Executing command in session container: " +
                cmdFlat +
                " with ENV: " +
                envFlat,
        );
    ...
    importContainerId(dockerContainerId) {
        if (dockerContainerId.length == 12) {
            this.app.addLog(
                "importContainerId was fed the short container id, expected full id.",
                "warn",
            );
        ...
        this.shortDockerContainerId = this.fullDockerContainerId.substring(
            0,
            12,
        );
    ...
    getContainerConfig() {
        ...
        if (devlopmentMode) {
            this.app.addLog(
                "Development mode enabled, mounting container-agent from local filesystem",
                "debug",
            );
            mounts.push({
                Source: this.app.absRootPath + "/external/container-agent/dist",
    ...
    async _checkContainerIsRunning() {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const found = containers.find(
                    (c) =>
                        c.id &&
                        c.id.substring(0, 12) === this.shortDockerContainerId,
                );
                ...
                if (found) {
                    ...
                    this.app.addLog(
                        `Container state check: State=${state} Status=${status}`,
                        "debug",
                    );
                    ...
                    if (
                        ...
                        this.app.addLog(
                            `Container ${this.shortDockerContainerId} is not running (State: ${state}, Status: ${status})`,
                            "error",
                        );
                        // You can add log tailing logic here if desired
                        throw new Error(
                            `Container in non-running state: ${status}`,
                        );
                    }
                ...
                this.app.addLog(
                    `Container ${this.shortDockerContainerId} not yet visible (attempt ${attempt}/${maxRetries}), retrying...`,
                    "debug",
                );
            } catch (err) {
                ...
                this.app.addLog(
                    `Error checking container list (attempt ${attempt}/${maxRetries}): ${err}`,
                    "warn",
                );
            }
        ...
        this.app.addLog(
            `Container ${this.shortDockerContainerId} disappeared from daemon.`,
            "error",
        );
    ...
    async _waitForSessionReady() {
        ...
        while (keepWaiting) {
            ...
            if (elapsed > maxWait) {
                throw new Error(
                    `Timeout waiting for session to become ready after ${elapsed} ms`,
                );
            }
            ...
            try {
                if (await this.isSessionReady()) {
                    this.app.addLog(
                        `Session is ready after ${Date.now() - startTime} ms`,
                    );
                    return; // Success, exit the loop
                }
            } catch (err) {
                this.app.addLog(
                    `isSessionReady threw an error: ${err}`,
                    "warn",
                );
            }
    ...
    async createContainer() {
        ...
        this.app.addLog(
            `${this.hsApp} ${this.user.username} ${this.project.id}`,
        );
        ...
        try {
            ...
            this.app.addLog(
                `Failed to create or start session container: ${error}${stack}`,
                "error",
            );
    ...
    setupLogStreaming() {
        try {
            ...
            this.container
                ...
                .then((stream) => {
                    ...
                    stream.on("data", (chunk) => {
                        try {
                            ...
                            this.app.addLog(
                                `Error writing container log chunk: ${err}`,
                                "warn",
                            );
                        }
                    });
                    stream.on("end", () => {
                        ...
                        this.app.addLog(
                            `Container log stream ended for ${this.shortDockerContainerId}`,
                            "debug",
                        );
                    });
                    stream.on("error", (err) => {
                        this.app.addLog(
                            `Container log stream error: ${err}`,
                            "warn",
                        );
                    });
                })
                .catch((err) => {
                    this.app.addLog(
                        `Failed to start container log stream: ${err}`,
                        "warn",
                    );
                });
        } catch (err) {
    ...
    async commit(branch = "master") {
        ...
        return new Promise((resolve) => {
            this.runCommand(
                ["node", "/container-agent/main.js", "save"],
                [
                    "GIT_USER_NAME=" +
                        this.user.firstName +
                        " " +
    ...
    async delete() {
        ...
        try {
            ...
            this.app.addLog(
                "Session error at proxy-server delete: " + error,
                "error",
            );
        }
        ...
        try {
            ...
            if (this.logFileWrite) {
                try {
                    this.logFileWrite.end();
                } catch (e) {
                    /* Ignore errors when ending write stream */
                }
            }
        } catch (err) {
        ...
        try {
            ...
            this.app.addLog(
                "Session error at container stop: " + error,
                "error",
            );
        }
